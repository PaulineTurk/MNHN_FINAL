...
name_folder=OUTPUT_SELECTION_PC_3D_biais_echantillon_$$
mkdir $name_folder
...
#for ((x=0; x<=9; x++)); 
    ...
    for j in ${myPseudoCounter[@]};
        do
        nohup python3 selection_pc_3d_biais_echantillon.py $pid_inf $pid_sup "uni" "o" $j > $name_folder/$name_file/o_uni_"$j".txt 2>&1 &
        done
